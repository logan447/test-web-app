...
| CC-34 | **CRITICAL: One unified request page framework** — All request pages (family↔provider, provider↔caregiver, caregiver↔org) must share: same core infrastructure, same design patterns, same functional flow. Only contextual copy/labels vary based on: who initiates, relationship type, engagement intent (tour/consultation/interview). Do NOT create separate page implementations—extend the Sprint 1 `/requests/[id]` canonical pattern. See design-standards.md for context table. | `UX` `VISUAL` `LOGIC` | Open |
